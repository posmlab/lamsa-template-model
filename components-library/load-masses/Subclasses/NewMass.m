...
classdef NewMass < Mass
    ...
    methods (Static)
        function parameters = parameters()
            parameters = ["mass" "mass of lever arm" "L1" "L2" "x_rest" "theta initial";
                "0.01" "0" "0.001" "0.001" "0.001" "0";
                "0" "0" "0" "0" "0" "0";
    ...
    methods
        ...
        function obj = NewMass(m_end,varargin)
            varargin_param_names = {'m_rod','L1','L2','x_rest','theta_0'};
...
%                 sqrt(1-(((x_rest-y(1))^2-(x_rest-y(2))^2)/(2*(x_rest-y(2))*L1))^2))...
%                 +1000000*(sqrt(L1^2+(x_rest-y(1))^2)>(x_rest-y(2)+L1));
%             mass = @(y) real((m_end/(EMA^2) + m_rod*( (1+1/EMA)^2 + 3*(1/EMA-1)^2 ) /12)/...
%                 sqrt(1-(((x_rest-y(1))^2-(x_rest-y(2))^2-2*(x_rest-y(1))*L1*sin(theta_0))/(2*(x_rest-y(2))*L1))))...
%                 +100*(sqrt(L1^2+(x_rest-y(1))^2)>(x_rest-y(2)+L1));
            mass = @(y) real((m_end/(EMA^2) + m_rod*( (1+1/EMA)^2 + 3*(1/EMA-1)^2 ) /12)/...
                sin(acos(((x_rest-y(2))^2-(x_rest-y(1))^2+2*(x_rest-y(1))*L1*sin(theta_0))/(2*(x_rest-y(2))*L1))))...
                +10000000*(sqrt((L1*cos(theta_0))^2+(x_rest-y(1)-L1*sin(theta_0))^2)>(x_rest-y(2)+L1));
            EMA = @(y) EMA;
