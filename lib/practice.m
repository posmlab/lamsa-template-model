...
xname = 'youngs modulus';
xrange = [4 9];
y_m = logspace(xrange(1),xrange(2),N);


%setting y axis value on plot 
yname = 'cross-sectional area';
yrange = [-7 -2];
c_s_a = logspace(yrange(1),yrange(2),N);
...
loading_motor_range_of_motion = .005;
v_max_loading_motor = 1;

% extra parameters for hill muscle motor
loading_motor_muscle_length = .01;
...
unlatching_motor = linear_motor(5,5,5);
