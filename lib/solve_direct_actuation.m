...
%actuated motion usinf F=ma. Returns [times, y or theta, velocities, force
% on load]
%motor struct
    %motor.Force
    %motor.max_force
...
launch_opts=odeset('Events',@(t,y) direct_actuation_end(t,y,motor, load, spring),'RelTol',1E-7,'AbsTol',1E-10);
...
for i = 1:size(T)
    fMotor(i) = F_eff(load, motor, spring, T(i), [Y(i,:)]);
end
fMotor = fMotor';
...
function [value,isterminal,direction]=direct_actuation_end(t,y,motor, load, spring)
