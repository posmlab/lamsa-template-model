...
%motor.Force = @(t,x) ((x(1)/muscle_length)<=1)*((x(1)/muscle_length)>=0.7)*Fmax_motor * exp(-((abs((((x(1)/muscle_length)^motor.b_L)-1)/motor.s))^motor.a_L)) * ((1-(x(2)/vmax_motor))/(1+(x(2)/(vmax_motor/4)))) .* (min(r_activation*t,1));
motor.Force = @(t,x) exp(-((abs((((x(1)/muscle_length)^motor.b_L)-1)/motor.s))^motor.a_L));
