...
motor.Force = @(t,x) Fmax_motor*(1-(x(2)/vmax_motor))/(1+(x(2)/(vmax_motor/4)))*(exp(-((abs(((((muscle_length-x(1))/muscle_length)^(motor.b_L))-1)/motor.s)^motor.a_L))))*min(r_activation*t,1);
